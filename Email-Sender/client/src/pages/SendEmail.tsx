...
// Convert display text back to HTML (for saving)
    ...
    if (drafts && drafts.length > 0) {
      ...
      if (defaultDraft) {
        ...
        setOriginalState({
          ...
          logoAttachmentId: defaultDraft.logoAttachmentId || null
        });
...
const handleOpenAttachment = async (attachment: EmailAttachment) => {
    ...
    try {
      if (currentDraftId) {
        ...
        await updateDraftMutation.mutateAsync({
          ...
          logoAttachmentId: logoAttachmentId === null ? null : logoAttachmentId,
  ...
  const generatePreview = (text: string) => {
    ...
    result = result.replace(/{{first_name}}/g, previewRecipient.firstName || "");
    result = result.replace(/{{last_name}}/g, previewRecipient.lastName || "");
